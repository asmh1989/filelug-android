package com.filelug.android.util;

import android.os.ParcelFileDescriptor;
import android.util.Log;

import java.io.IOException;
import java.io.InputStream;
import java.io.OutputStream;

/**
 * Created by Vincent Chang on 2016/05/25.
 * Copyright (c) 2016 Filelug. All rights reserved.
 */
public class ParcelFileDescriptorUtil {

    private static final String TAG = ParcelFileDescriptorUtil.class.getSimpleName();

    public static ParcelFileDescriptor pipeFrom(InputStream inputStream) throws IOException {
//        if ( Constants.DEBUG ) Log.d(TAG, "pipeFrom()");
        ParcelFileDescriptor[] pipe = ParcelFileDescriptor.createPipe();
        ParcelFileDescriptor readSide = pipe[0];
        ParcelFileDescriptor writeSide = pipe[1];

        // start the transfer thread
        new TransferThread(
            inputStream,
            new ParcelFileDescriptor.AutoCloseOutputStream(writeSide)
        ).start();

        return readSide;
    }

    public static ParcelFileDescriptor pipeTo(OutputStream outputStream) throws IOException {
//        if ( Constants.DEBUG ) Log.d(TAG, "pipeTo()");
        ParcelFileDescriptor[] pipe = ParcelFileDescriptor.createPipe();
        ParcelFileDescriptor readSide = pipe[0];
        ParcelFileDescriptor writeSide = pipe[1];

        // start the transfer thread
        new TransferThread(
            new ParcelFileDescriptor.AutoCloseInputStream(readSide),
            outputStream
        ).start();

        return writeSide;
    }

    static class TransferThread extends Thread {
        final InputStream mIn;
        final OutputStream mOut;

        TransferThread(InputStream in, OutputStream out) {
            super("ParcelFileDescriptor Transfer Thread");
            mIn = in;
            mOut = out;
            setDaemon(true);
//            if ( Constants.DEBUG ) Log.d(TAG, "new TransferThread()");
        }

        @Override
        public void run() {
            byte[] buf = new byte[1024];
            int len;
            int size = 0;

            try {
//                if ( Constants.DEBUG ) Log.d(TAG, "new TransferThread().run(), Start to write!");
                while ((len = mIn.read(buf)) > 0) {
                    mOut.write(buf, 0, len);
                    size += len;
                }
                mOut.flush();
//                if ( Constants.DEBUG ) Log.d(TAG, "new TransferThread().run(), size=" + size + ", length=" + len);
            } catch (IOException e) {
                Log.e(TAG, "Exception transferring file", e);
            } finally {
                try {
                    mIn.close();
                } catch (IOException e) {
                    Log.e(TAG, "Exception closing input stream", e);
                }
                try {
                    mOut.close();
                } catch (IOException e) {
                    Log.e(TAG, "Exception closing output stream", e);
                }
            }
        }
    }

}
